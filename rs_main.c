...
Long encode_rs012(uint8_t *data, uint8_t *parity, int32_t length);
#define DATA_SIZE 30
...
int main() {
    ...
    run_tests(rs, testbench, block_length, message_length, min_distance / 2, 0, 20000);
    run_tests(rs, testbench, block_length, message_length, 0, min_distance, 20000);
    run_tests(rs, testbench, block_length, message_length, min_distance / 4, min_distance / 2, 20000);
