// Tea testing
#include "tea.h"
#include "encode.h"
#include "printers.h"
#include "parameters.h"
#include <string.h>
#include <stdlib.h>

#define view 80

void init_decoder();
void test_mls();
void test_rs012();
void test_convo();
void init_zc();
void serve_tea();
Long bit_sequenced(Byte * frame, Short n, Byte * bits);
Long find_bit_sync(Byte * bits);
Long find_frame_sync(Byte * bits);
Short bits_to_bytes(Byte * bits, Byte * bytes, Short len);
int deconvolve_buffer(Byte * frame, Byte * decoded, Short length);
int decode_rs(Byte * data, Long size);
Byte * trim_bits(Byte * seq, Short bits);

// Short count_bits(Byte * bits, Short nbits);
// Short match_bit_seqs(Byte * sync, Byte * frame);

static Byte alpdu1[] = {0x00, 0x00, 0x10, 0x0B, 0x0B, 0xB8, 0x44, 0x5A, 0xEC,
                        0x01, 0x06, 0x08, 0x11, 0x84, 0xC9, 0x11, 0x04};
static Byte alpdu2[] = {0x04, 0x00, 0x10, 0x16, 0x03, 0xE8, 0x00, 0x02, 0x04,
                        0x00, 0x12, 0x00, 0x00, 0x01, 0x09, 0x08, 0x11, 0x03,
                        0x0A, 0x11, 0x00, 0x09, 0x11, 0x00, 0x05, 0x02, 0x20,
                        0x40};
static Byte alpdu3[] = {};

extern Byte bit_sync_bits[BSYNC_SIZE * 8];
extern Byte frame_sync_bits[3][FSYNC_SIZE * 8];

static Byte fec = 0, pdu = 0;
static Byte * pdus[3] = {alpdu1, alpdu2, alpdu3};
static Long pdu_size[3] = {sizeof(alpdu1), sizeof(alpdu2), sizeof(alpdu3)};
static Byte * test_pdu = alpdu1;
static Long test_size = sizeof(alpdu1);
static Long success = 0;
static Long tests = 0;

static void next_test() {
    // print("\n==== Encode an airlink pdu for decoding testing in fec mode: ");
    // printDec(fec);
    // print(" airpdu: "), printDec(test_size);
    // print("bytes  ");//, hbytes(test_pdu, min(40,test_size));
    set_value(FEC_Mode, fec);
    encode_airpdu(test_pdu, test_size);
}

static void finish_test() {
    tests++;
    if (fec == 2 && test_size == 1023) {
        print("\nNumber of tests, successess: "), printDec(tests), printDec(success);
        return;
    }
    fec = (fec + 1 ) % 3;
    if (fec == 0)
        test_size++;
    later(next_test);
}

void fec2_bits(Long n) { // n bytes is how many coded bits with tail and punctures
    printCr(), printDec(n);
    n = n * 8 + 9; // number of bits plus tail
    Byte punc = 0;
    Long bits = 0;
    while (n--) { // convolve with punctures
        switch (punc) {
        case 0: bits += 2; punc = 1; break;
        case 1: bits += 1; punc = 2; break;
        case 2: bits += 1; punc = 0; break;
        }
    }
    print("bytes convolved with tail becomes "), printDec(bits), print("bits  ");
}

void calc_seg1(AirFrame * afd) {
    Long offset = find_bit_sync(afd->bits);
    if (offset >= afd->nbits) {
        print("  No bit sync match ");
        return;
    }

    afd->bits = trim_bits(afd->bits, offset);
    afd->nbits -= offset; // recalculate length after removing syncs

    afd->nbytes = bits_to_bytes(afd->bits, afd->bytes, afd->nbits);

    afd->fec = find_frame_sync(afd->bytes + BSYNC_SIZE);
    if (afd->fec > 2) { print("\n Cannot decode further! Bad FEC mode!"); return; }

    afd->nrs = afd->fec == 0 ? FEC0_NROOTS : FEC12_NROOTS; // nroots
    afd->seg1 = afd->bytes + SYNC_SIZE;
    afd->nseg1 = afd->fec == 0 ? SEG10_SIZE : afd->fec == 1 ? SEG11_SIZE : SEG12_SIZE;
    afd->nseg2 = afd->nbytes - SYNC_SIZE > afd->nseg1 ? afd->nbytes - SYNC_SIZE - afd->nseg1 : 0;
}

void create_airpdu(AirFrame * afd, Byte * seg1, Byte * airpdu) {
    Short nseg1 = min(afd->len, afd->nseg1 - 2);
    memcpy(airpdu, seg1 + 2, nseg1);

    if (afd->nseg2) {
        Byte seg2[afd->nseg2];
        bool valid = true;
        switch(afd->fec) {
        case 0:  valid = afd->nseg2 >= MIN_SEG10_SIZE && afd->nseg2 < 10000; break;
        case 1:  valid = afd->nseg2 >= MIN_SEG11_SIZE && afd->nseg2 < 10000; break;
        case 2:  valid = afd->nseg2 >= MIN_SEG12_SIZE && afd->nseg2 < 10000; break;
        }
        if (valid) {
            int n = deconvolve_buffer(afd->seg2, seg2, afd->nseg2*8);
            if (n < 0) { print("\n Faild deconvolving"); return; }
            afd->nseg2 = n;
            Byte blockm = afd->fec == 0 ? BLOCKm0 : afd->fec == 1 ? BLOCKm1 : BLOCKm2;
            Byte rsblock = blockm + afd->nrs;
            for (Short s = 0, d = 0; s < afd->nseg2; s += rsblock, d += blockm) {
                int n = min(rsblock, afd->nseg2 - s) - afd->nrs;
                if (n > 0) {
                        decode_rs(seg2 + s, n);
                    unscramble_blk(seg2 + s, n);
                    memcpy(airpdu + nseg1 + d, seg2 + s, n);
                } else
                    printDec(n);
            }
        } else
            print(" Segment Wrong size! ");
    }
}

void continue_test() {
    AirFrame * afe = get_airframe();
    Byte bits[8 * afe->nbytes];
    memset(bits, 0, sizeof(bits));
    AirFrame afd = {.bits = bits, .nbits = bit_sequenced(afe->bytes, afe->nbits, bits)};
    Byte bytes[afd.nbits+1];
    afd.bytes = bytes;
    calc_seg1(&afd);
    Byte seg1[afd.nseg1];
    afd.seg2 = afd.nseg2 ? afd.seg1 + afd.nseg1 : NULL;
    afd.nseg1 = deconvolve_buffer(afd.seg1, seg1, afd.nseg1*8) - afd.nrs;
    if (afd.nseg1 < 0) { print("\n Faild deconvolving"); return; }
    decode_rs(seg1, afd.nseg1);
    unscramble_blk(seg1, afd.nseg1);

    // Assemble block 1
    afd.len = seg1[0] << 8 | seg1[1];
    if (afd.len > 1023) { print("\n Length is too big, aborting decoding"); return; }
    Byte airpdu[afd.len];
    create_airpdu(&afd, seg1, airpdu);
    if (afd.len == test_size && memcmp(airpdu, test_pdu, test_size) == 0)
        success++;
    else {
        print("  Different! ");
        for (Short i = 0; i < test_size; i++)
            if (airpdu[i] != test_pdu[i]) {
                print("at byte "), printDec(i);
                break;
            }
    }
    finish_test();
}

void frame_test() {
    test_pdu = pdus[2];
    test_size = 1;
    pdu = 0;
    fec = 0;
    when(mants_encoded, continue_test);
    later(next_test);
}

int main() {
    init_tea();
    init_zc(); // from init_comps
    init_decoder();
    // later(test_mls);
    // later(test_rs012);
    // later(test_convo);
    later(frame_test);
    fec2_bits(74);
    serve_tea();
    return 0;
}

// static char *allPp[] = {    // GF_BITS  polynomial      */
// NULL,           /*  0   no code         */
// NULL,           /*  1   no code         */
// "111",          /*  2   1+x+x^2                          0x07*/ 
// "1101",         /*  3   1+x+x^3                          0x0B*/
// "1100 1",            /*  4   1+x+x^4                     0x13*/
// "1010 01",           /*  5   1+x^2+x^5                   0x25*/
// "1100 001",          /*  6   1+x+x^6                     0x43*/
// "1001 0001",         /*  7   1 + x^3 + x^7               0x89*/
// "1011 1000 1",        /*  8   1+x^2+x^3+x^4+x^8         0x11D*/
// "1000 1000 01",       /*  9   1+x^4+x^9                 0x211*/
// "1001 0000 001",      /* 10   1+x^3+x^10                0x409*/
// "1010 0000 0001",     /* 11   1+x^2+x^11                0x805*/
// "1100 1010 0000 1",        /* 12   1+x+x^4+x^6+x^12    0x1053*/
// "1101 1000 0000 01",       /* 13   1+x+x^3+x^4+x^13    0x201B*/
// "1100 0010 0010 001",      /* 14   1+x+x^6+x^10+x^14   0x4443*/
// "1100 0000 0000 0001",     /* 15   1+x+x^15            0x8003*/
// "1101 0000 0000 1000 1"     /* 16   1+x+x^3+x^12+x^16 0x1100B*/
// };
// 0001 1000 0111
// 1100 0011