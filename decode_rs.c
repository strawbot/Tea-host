 ...
 * into a function body with the following typedefs, macros and variables
 supplied
 ...
 * MODNN - a function to reduce its argument modulo NN. May be inline or a
 macro.
 * FCR - An integer literal or variable specifying the first consecutive root of
 the
 ...
 * DEBUG - If set to 1 or more, do various internal consistency checking. Leave
 this
 *         undefined for production code

 * The memset(), memmove(), and memcpy() functions are used. The appropriate
 header
 * file declaring these functions (usually <string.h>) must be included by the
 calling
 * program.
 */
...
int decode_rs012(struct rs *rs, data_t *data, int *eras_pos) {
    ...
    data_t root[NROOTS], reg[NROOTS + 1];
    int syn_error, count;
    ...
    for (j = count - 1; j >= 0; j--) {
        ...
        /* Apply error to data */
        if (num1 != 0 && eras_pos[j] >= PAD) {
            data[eras_pos[j] - PAD] ^= ALPHA_TO[MODNN(
